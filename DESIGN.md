...
## Project Theme — k8s-journey

本项目沿用上面的 Geist / Vercel 设计语言（近白画布、墨色文字、1px 发丝边框、Geist Sans + Geist Mono），
只把强调色从 Vercel Blue 换成 **Kubernetes Blue**，呼应 K8s 品牌：

| Token | Value | Use |
|---|---|---|
| `accent` | `#326ce5` | 链接、焦点环、当前章节高亮、进度条 |
| `accent-deep` | `#2556c4` | hover / pressed |
| `accent-soft` | `#e4ecfc` | 提示框底色、选中态浅底 |

- 首屏 hero 的网格渐变以 `#326ce5 → #50e3c2 → #7928ca` 为主，其余界面保持黑白。
- 学习阶段徽标使用 Geist Mono 大写 eyebrow（如 `STAGE 01 · 入门`）。
